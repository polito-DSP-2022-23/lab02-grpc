...
module.exports.getSingleImage = function getSingleImage (req, res, next) {
  ...
  Images.getSingleImage(req.params.imageId, req.params.filmId, req.user.id)
    .then(function (response) {
       utils.writeJson(res, response);
    })
    .catch(function (response) {
      if(response == 403){
        utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': 'The user is not the owner or reviewe of the film' }], }, 403);
      }
      else if (response == 404){
        utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': 'The resource does not exist.' }], }, 404);
      }
...
module.exports.getSingleImageFile = function getSingleImageFile (req, res, next) {
  ...
  Images.getSingleImageFile(req.params.imageId, imageType, req.params.filmId, req.user.id)
    .then(function (response) {
